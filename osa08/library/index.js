...
const resolvers = {
  Query: {
    ...
    allBooks: (root, args) => {
      const booksToReturn = args.author
  ...
  Mutation: {
    addBook: (root, args) => {
      const bookToCreate = { ...args, id: uuid() }
      books = [...books, bookToCreate]
      if (!authors.find(author => author.name === bookToCreate.author)) {
        const authorToCreate = { name: bookToCreate.author, id: uuid() }
        authors = [...authors, authorToCreate]
      }
      return bookToCreate
    },
